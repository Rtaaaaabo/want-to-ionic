...
export class MemberListPage implements OnInit {
  ...
  searchValue(ev: CustomEvent): void {
    ...
    let resultUsernameData: Array<any>;
    let resultEmailData: Array<any>;
    this.logic.fetchUsernameFilteredCompanyMembers(this.currentUser.companyID, searchContent)
      .pipe(map((result) => resultUsernameData = result))
      ...
      .subscribe(() => {
        const test = resultUsernameData.filter((usernameData) => {
          return resultEmailData.filter((emailData) => {
            usernameData.id === emailData.id;
          });
        });
        this.companyMembers = resultUsernameData.concat(resultEmailData);
      });
  }
